    ...
    constructor(auth) {
        this.fetchProfilePicture = async () => {
            ...
            try {
                ...
                const profile_picture = await betro_js_lib_1.symDecryptBuffer(this.auth.symKey, data);
        ...
        this.whoAmi = async () => {
            ...
            if (data.first_name != null) {
                const first_name_bytes = await betro_js_lib_1.symDecryptBuffer(this.auth.symKey, data.first_name);
                first_name = first_name_bytes === null || first_name_bytes === void 0 ? void 0 : first_name_bytes.toString("utf-8");
            }
            if (data.last_name != null) {
                const last_name_bytes = await betro_js_lib_1.symDecryptBuffer(this.auth.symKey, data.last_name);
        ...
        this.fetchProfile = async () => {
            try {
                ...
                if (symDecrypted != null) {
                    const first_name = await betro_js_lib_1.symDecryptBuffer(symDecrypted, encrypted_first_name);
                    const last_name = await betro_js_lib_1.symDecryptBuffer(symDecrypted, encrypted_last_name);
                    const profile_picture = await betro_js_lib_1.symDecryptBuffer(symDecrypted, encrypted_profile_picture);
                    if (first_name == null ||
                        last_name == null ||
        ...
        this.createProfile = async (first_name, last_name, profile_picture) => {
            try {
                const encrypted_sym_key = await betro_js_lib_1.symEncrypt(this.auth.encryptionKey, this.auth.symKey);
                const encrypted_first_name = await betro_js_lib_1.symEncrypt(this.auth.symKey.toString("base64"), Buffer.from(first_name));
                const encrypted_last_name = await betro_js_lib_1.symEncrypt(this.auth.symKey.toString("base64"), Buffer.from(last_name));
                const request = {
                ...
                if (profile_picture != null) {
                    const encrypted_profile_picture = await betro_js_lib_1.symEncrypt(this.auth.symKey.toString("base64"), profile_picture);
        ...
        this.updateProfile = async (first_name, last_name, profile_picture) => {
            try {
                const request = {};
                if (first_name != null) {
                    request.first_name = await betro_js_lib_1.symEncrypt(this.auth.symKey.toString("base64"), Buffer.from(first_name));
                }
                if (last_name != null) {
                    request.last_name = await betro_js_lib_1.symEncrypt(this.auth.symKey.toString("base64"), Buffer.from(last_name));
                }
                if (profile_picture != null) {
                    request.profile_picture = await betro_js_lib_1.symEncrypt(this.auth.symKey.toString("base64"), profile_picture);
                }
